...
- **Alert/Dialog:** SweetAlert **yalnızca `src/utils/alert.js`'te** kullanılır ve bu kural lint'te `no-restricted-imports` ile zorlanır (§3). Sayfalar `sweetalert2`'yi import etmez, `showAlert` metodlarını çağırır. Yeni bir dialog gerekiyorsa `alert.js`'e metod ekle.
  ...
  - Dialog stilleri `style.css`'te `swal-*` sınıflarında tutulur, üretilen HTML'e inline `style` yazma. Renkler oradaki tema değişkenlerinden gelir, `swal-*` kurallarında hex sabitleme.
...
Koruma iki katmandır: `ProtectedRoute` (girişli/girişsiz) ve `RequireBuilding` (seçili bina var mı). **İkisi de layout route'tur:** `children` almazlar, `<Outlet />` render ederler ve `<Route element={<ProtectedRoute />}>` biçiminde bir rota grubunu sararlar. Yeni bir guard da bu şekilde yazılır, rota başına sarmalayıcı tekrarlanmaz.
