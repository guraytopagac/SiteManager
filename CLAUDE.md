...
└── src/
    ...
    ├── pages/           # Her sayfa kendi klasöründe ve o klasörde yalnızca iki dosya vardır:
    │                    #   <Sayfa>.jsx + <Sayfa>.css. App.jsx'te lazy-load
    ├── utils/           # alert.js, date.js, currency.js, passwordPolicy.js, releaseNotes.js
...
**Sayfa klasörü iki dosyadır: `<Sayfa>.jsx` ve `<Sayfa>.css`.** Üçüncü bir dosya açılmaz. Sayfaya özgü bir alt bileşen ya da hook gerekiyorsa **dosya-yerel** tanımlanır ve sayfanın kendi `.jsx`'inde durur, ayrı dosyaya çıkarılmaz. `Recover.jsx`'in `ErrorNotice`'ı ile `SelectBuilding.jsx`'in `renderNameForm`'u bunun örneğidir, `AuthField.jsx`'in `CapsLockBadge`'i de aynı desendir. Bir parçayı gerçekten birden fazla **sayfa** kullanıyorsa yeri `src/components/`, birden fazla tüketicisi olan bir hook'un yeri `src/hooks/`'tur. Ölçüt tüketici sayısıdır, dosyanın uzunluğu değil.

**Apartments bu kuralın tek ihlalidir.** `Apartments.jsx` hem aidat/daire listesini hem tüm daire işlemlerini (tahsilat, düzenleme, pasife alma, toplu aidat) barındırır, ama ortak parçalarını `components/` altında, sabitlerini `constants.js`'te ve aidat veri çekme mantığını `useDues.js`'te taşır. Kurala getirilmesi `ROADMAP.md`'dedir ve o güne kadar §9'un `constants.js` göndermesi geçerlidir. Yeni sayfa yazarken bu klasör örnek alınmaz.
...
12. **Dashboard metrikleri (`getStats`).** Üç ölçümün tanımı da servistedir ve üçü de yalnızca **aktif** daireleri sayar, yani pasife alınmış bir dairenin kural 7 gereği tabloda duran borcu bu ölçümlere girmez.
