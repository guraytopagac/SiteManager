...
INSERT INTO incomes_new (id, building_id, due_payment_id, amount, date, description, category,
                         is_cancelled, cancelled_at, cancel_reason, cancelled_by, created_at, updated_at)
SELECT i.id, b.id, i.due_payment_id, i.amount, i.date, i.description, i.category,
       i.is_cancelled, i.cancelled_at, i.cancel_reason, i.cancelled_by, i.created_at, i.updated_at
FROM incomes i
JOIN buildings b ON b.owner_id = i.manager_id;
